...
from app.services.ai_service import ollama_provider, AIService

def test_prompt_building():
    prompt = ollama_provider._build_prompt("space-weather", "Class X Solar Flare")
    assert "space weather" in prompt.lower()
    
    prompt_neo = ollama_provider._build_prompt("neo", "Asteroid approaching")
    assert "orbital hazard" in prompt_neo.lower()

    prompt_launch = ollama_provider._build_prompt("launch", "Falcon 9 heading to orbit")
...
async def test_explanation_generation_fresh():
    with patch("app.services.ai_service.redis_client.get", return_value=None) as mock_redis_get, \
         ...
         patch("app.services.ai_service.ollama_provider.explain", return_value="A critical solar flare was observed.") as mock_explain:
...
async def test_explanation_caching():
    with patch("app.services.ai_service.redis_client.get", return_value="Cached solar storm summary") as mock_redis_get, \
         patch("app.services.ai_service.ollama_provider.explain") as mock_explain:
