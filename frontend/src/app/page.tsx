...
export default function Home() {
  ...
  return (
    <div className="relative w-screen h-[100dvh] overflow-x-hidden overflow-y-auto lg:overflow-hidden flex flex-col">
      {/* 3D Globe Underlay */}
      ...
      <div className={`fixed bottom-[84px] left-4 right-4 lg:relative lg:absolute lg:left-4 lg:top-24 lg:bottom-4 lg:w-80 z-40 flex-col gap-4 mt-24 lg:mt-0 px-0 lg:pr-1 overflow-y-auto scrollbar-none flex-none pointer-events-auto transition-transform duration-300 scale-[0.85] lg:scale-100 origin-bottom lg:origin-top-left ${activeMobilePanel === 'iss' ? 'flex' : 'hidden lg:flex'}`}>
        ...
        <div className="rounded-xl border border-white/10 bg-black/80 lg:bg-black/40 backdrop-blur-md p-4 shadow-[0_4px_20px_rgba(0,0,0,0.5)] space-y-3">
          ...
          <Button
            ...
            className={`w-full h-11 lg:h-9 font-mono text-xs gap-2 transition-all duration-300 ${trackISS
      ...
      <ErrorBoundary>
        <PanelStack className={`fixed bottom-[84px] left-4 right-4 lg:relative lg:absolute lg:right-4 lg:top-24 lg:bottom-4 lg:w-96 z-40 flex-col gap-4 pb-0 lg:pb-8 lg:p-0 lg:pr-1 overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent flex-none pointer-events-auto transition-transform duration-300 scale-[0.85] lg:scale-100 origin-bottom lg:origin-top-right ${activeMobilePanel && activeMobilePanel !== 'iss' ? 'flex' : 'hidden lg:flex'}`}>
          ...
          {(activeMobilePanel === 'neo' || !activeMobilePanel) && (
            <div className={`flex-col gap-4 ${activeMobilePanel === 'neo' ? 'flex' : 'hidden lg:flex'}`}>
              <div className="flex justify-between items-center lg:hidden mb-1 px-2 bg-black/60 rounded-xl p-3 border border-white/10 backdrop-blur-md">
          ...
          {(activeMobilePanel === 'launches' || !activeMobilePanel) && (
            <div className={`flex-col gap-4 ${activeMobilePanel === 'launches' ? 'flex' : 'hidden lg:flex'}`}>
              <div className="flex justify-between items-center lg:hidden mb-1 px-2 bg-black/60 rounded-xl p-3 border border-white/10 backdrop-blur-md">
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 hidden lg:flex items-center gap-2 px-3 py-1.5 rounded-full border border-white/5 bg-black/30 backdrop-blur-sm text-[9px] font-mono text-zinc-500">
        <Radio className="h-3 w-3 text-cyan-500 animate-pulse" />
        <span>SECURE COM LINK ESTABLISHED</span>
      </div>

      {/* Mobile Bottom Navigation Bar */}
      <div className="fixed bottom-0 left-0 right-0 z-50 flex lg:hidden bg-black/80 backdrop-blur-xl border-t border-white/10 p-2 pb-4 justify-around items-center">
